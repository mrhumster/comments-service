...
import (
	...
	"github.com/mrhumster/comments-service/internal/database"
	"github.com/mrhumster/comments-service/internal/delivery/http/routes"
...
func main() {
	figure.NewFigure("comments "+version, "graffiti", true).Print()

	opts := &slog.HandlerOptions{Level: slog.LevelDebug, AddSource: true}
	slog.SetDefault(slog.New(slog.NewTextHandler(os.Stdout, opts)))
	slog.Info("Start comments reader", "version", version, "build_date", buildDate)
	...
	go func() {
		log.Printf("comments reader listening on %s", cfg.Server.ServerAddr)
	...
	<-quit
	log.Println("shutting down comments reader...")
